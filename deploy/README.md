...
Nginx 公开高清品牌图、预览图和缩略图目录，原始文件只在服务器磁盘保存，不通过 Nginx
公开。官网列表和封面使用缩略图或预览图，作品灯箱按需加载高清品牌图。
...
Flyway 执行，常规发布不停止服务导出或备份数据库。应用回滚不会自动回退数据库 schema，
因此 Flyway 迁移必须采用可向前执行、与上一版应用兼容的增量设计。
